module.exports = {
    ...
    run: async (sock, m, { clientPerms, config, localDB, text }) => {
        ...
        // ⚡ COUNT PLUGINS: kama all, use total commands
        let totalPlugins = userPermissions.includes("all") 
            ? Object.values(menuCategories).flat().length
        ...
        for (const [category, commands] of Object.entries(menuCategories)) {
            ...
            if (userPermissions.includes("all") || isOwner) {
                filteredCmds = commands; // show all
            } else {
