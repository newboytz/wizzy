module.exports = {
    ...
    run: async (sock, m, { clientPerms, config, localDB, text }) => {
        ...
        try {
            if (pichaYaKutuma) {
                // Jaribu kutuma na picha
                return await sock.sendMessage(chat, {
                    image: { url: pichaYaKutuma },
                    caption: dynamicMenu
                }, { quoted: m });
            } else {
                // Kama hakuna picha kabisa kwenye config, tuma maandishi tupu
                throw new Error("No Image"); 
            }
        } catch (err) {
            // Kama picha imegoma (Error 404, etc.) au haipo, tuma menu ya maandishi pekee
